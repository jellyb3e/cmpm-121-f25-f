...
const Room1Scene = () => {
    ...
    const WALL_COLOR = 0x4b9963;
    const GROUND_COLOR = 0x215e34;
    ...
    const makeRoom = () => {
        // floor
        physics.add.box({ x: 0, y: 0, z: 0, width: 20, height: 1, depth: 20, mass: 0 }, { lambert: { color: GROUND_COLOR } }).body.setCollisionFlags(2);
        // walls
        physics.add.box({ x: 10.5, y: 2, z: 0, width: 1, height: 5, depth: 22, mass: 0 }, { lambert: { color: WALL_COLOR } }).body.setCollisionFlags(2);
        physics.add.box({ x: -10.5, y: 2, z: 0, width: 1, height: 5, depth: 22, mass: 0 }, { lambert: { color: WALL_COLOR } }).body.setCollisionFlags(2);
        physics.add.box({ x: 0, y: 2, z: 10.5, width: 20, height: 5, depth: 1, mass: 0 }, { lambert: { color: WALL_COLOR } }).body.setCollisionFlags(2);
        physics.add.box({ x: 0, y: 2, z: -10.5, width: 20, height: 5, depth: 1, mass: 0 }, { lambert: { color: WALL_COLOR } }).body.setCollisionFlags(2);
    }
    makeRoom();

    // PLAYER
    const player = physics.add.capsule({ x: 0, y: 1, z: 0, radius: 0.75, length: 1, mass: 1 }, { lambert: { color: HAND_COLOR } });
